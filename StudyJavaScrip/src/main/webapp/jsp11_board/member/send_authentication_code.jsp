...
String content = "<h3>인증하려면 아래링크를 클릭하세요</h3><a href='http://localhost:8080/StudyJSP/jsp11_board/member/member_authentication.jsp?id="+ id + "&code=" + code +"'>인증하기</a>";
...
try {
	...
	// 2. 수신자 정보 설정
	Address email_address = new InternetAddress(email);
	...
	mailMessage.addRecipient(RecipientType.TO, email_address);

	// 4) 메일 제목 설정
	mailMessage.setSubject("인증번호");

	// 5) 메일 본문 설정
	mailMessage.setContent(code, "text/html; charset=UTF-8");
